...
function MaterialPreviewModal({ material, onClose }) {
  ...
  // Flexible PDF detector across all possible properties
  const isPdf =
    material.fileMimeType === "application/pdf" ||
    material.type === "pdf" ||
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 space-y-4 border border-slate-100 shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex-grow overflow-hidden rounded-2xl border border-slate-200 min-h-[320px] bg-slate-50 flex flex-col justify-center">
          {status === "loading" && (
            <div className="flex flex-col items-center justify-center gap-2 text-slate-400">
              <Loader2 className="h-6 w-6 animate-spin" />
              <span className="text-xs">Loading material file...</span>
            </div>
          )}
...
export default function TeacherContentPublisherTab({
  ...
  const handleFileChange = (e) => {
    ...
    const reader = new FileReader();
    reader.onload = (uploadEvent) => {
      setFileData(uploadEvent.target.result);
    };
    reader.readAsDataURL(file);
    ...
    try {
      ...
      const newMaterial = {
        ...
        title: titleAlreadyHasExtension
          ...
          : `${trimmedTitle}${extension}`,
        size: formatFileSize(selectedFile.size),
        type: extension.replace(".", "").toLowerCase() || "pdf",
        readTime,
        content: guideContent.trim(),
        fileName: selectedFile.name,
        ...
        fileCacheKey: cacheKey,
        fileData: fileData,
        file_url: fileData, // 👈 Ensures full cross-compatibility with Student Portal!
      };
